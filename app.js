...
async function saveCharacter() {
    try {
        ...
        const stats = {
            core: typeof getStats === "function" ? getStats("core") : (oldChar?.stats?.core || {}),
            vital: typeof getStats === "function" ? getStats("vital") : (oldChar?.stats?.vital || {}),
            offense: typeof getStats === "function" ? getStats("offense") : (oldChar?.stats?.offense || {}),
            defense: typeof getStats === "function" ? getStats("defense") : (oldChar?.stats?.defense || {}),
            hidden: {
...
async function editCharacter(id) {
    ...
    if (c.stats?.hidden) {
        ...
        Object.keys(hMap).forEach(key => {
            ...
            const val = c.stats.hidden[hMap[key]] || 0;
    ...
    if (relContainer) {
        ...
        if (Array.isArray(c.relations)) {
            c.relations.forEach(r => { if (typeof addRelationField === "function") addRelationField(r); });
        }
    }
    ...
    if (formsContainer) {
        ...
        if (Array.isArray(c.forms)) {
            for (const f of c.forms) { if (typeof addFormField === "function") await addFormField(f); }
        }
    }
    const preview = document.getElementById("previewImg");
    if (preview) {
        if (c.img) {
            ...
            preview.src = isExt ? c.img : (typeof getImage === "function" ? await getImage(c.img).catch(() => "https://i.imgur.com/6X8FQyA.png") : "https://i.imgur.com/6X8FQyA.png");
            preview.classList.remove("hidden");
        } else {
            preview.src = "https://i.imgur.com/6X8FQyA.png";
            preview.classList.add("hidden");
...
async function openProfile(id) {
    ...
    charPage.innerHTML = `
        <style>
            ...
            /* Cập nhật desc-area để hiển thị đoạn văn tốt hơn */
